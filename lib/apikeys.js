...
// 모델이 없다고 판명된 (키, 모델) 은 잠시 기억해 매번 헛수고하지 않는다.
// 메모리 상태라 재컴파일하면 초기화된다 — 그 사이 모델이 열렸으면 다시 잡힌다.
var MODEL_DOWN_MS = 60 * 60 * 1000;   // 1시간
var _modelDown = {};                  // "key|model" -> 만료 시각

function nowMs() { return java.lang.System.currentTimeMillis(); }

// 이 응답이 "키 문제" 가 아니라 "모델 문제" 인가.
// 이걸 먼저 가려내지 않으면, 없는 모델을 부른 400/404 가 "잘못된 키" 로 분류돼
// 멀쩡한 키를 차례로 버리다가 전부 죽었다고 결론 내린다.
function isModelError(code, raw) {
  var s = String(raw || "");
  if (code === 404) return true;
  if (code === 400 && /api[\s_-]*key/i.test(s)) return false;   // 키 문제가 우선
  return /is not found for API version|NOT_FOUND|not supported for|models\/[^\s"]+ is not found/i.test(s);
}

function markModelDown(key, model, at) {
  _modelDown[String(key) + "|" + String(model)] = (at || nowMs()) + MODEL_DOWN_MS;
}

function isModelDown(key, model, at) {
  var until = _modelDown[String(key) + "|" + String(model)];
  return !!until && until > (at || nowMs());
}
