...
module.exports = function subscribe(botName, workerName, onMessage) {
  ...
  // ─── 메시지 큐 ─────────────────────────────────────────────────────
  // 유계(bounded) 큐. 워커가 죽어 소비자가 사라지면 ChatManager broadcast 의 offer() 가
  // 가득 찬 시점부터 실패(드롭)하므로, 죽은 구독봇 큐가 무한정 메모리를 먹던 문제를 막는다.
  // 살아있는 소비자는 즉시 take() 로 비우므로 실사용에서 가득 찰 일이 없다.
  var msgQueue = new java.util.concurrent.LinkedBlockingQueue(2000);
