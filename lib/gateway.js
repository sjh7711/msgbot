...
//
//   게이트웨이는 동시 1건만 처리한다. 붙잡혀 있으면 호출 측이 상태를 구분할 수 있도록
//   타임아웃을 짧게 두고 {error}를 돌려준다. 현재 맞춤 토픽 출제는 정확성 우선이라
//   이 오류를 받으면 모델 기억으로 폴백하지 않고 안전하게 출제를 중단한다.
//
...
var READ_TIMEOUT    = 20000;   // 평균 5초. 초과 시 호출 측에 오류를 반환한다
var MAX_QUERY = 300;           // 서버 query max_length와 동일
...
//   반환 실패: { error }   — 근거 필수 여부와 사용자 안내는 호출 측 정책이 결정한다
function search(query, maxResults) {
  var q = String(query == null ? "" : query).replace(/\s+/g, " ");
  q = q.replace(/^\s+|\s+$/g, "");
  // 문장 중간을 자르면 검색 의도가 달라질 수 있으므로 호출 측 builder 오류로 처리한다.
  if (q.length > MAX_QUERY) return { error: "질의가 너무 깁니다(최대 " + MAX_QUERY + "자)." };
