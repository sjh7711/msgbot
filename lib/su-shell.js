  ...
  try {
    ...
    try {
      ...
      while ((line = stdout.readLine()) !== null) {
        var s = String(line);
        if (s.indexOf(sentinel) !== -1) { sawSentinel = true; break; }
        sb.append(s).append("\n");
      }
      if (!sawSentinel) {
        // readLine null = EOF = 셸이 명령 도중 죽음(Magisk revoke/OOM 등).
        // 부분 출력을 정상 결과처럼 반환하면 호출부가 짧은 데이터를 참으로 오인하므로,
        // 죽은 셸을 정리하고 오류를 표면화한다(다음 exec 가 셸을 재개).
        try { h.get("proc").destroy(); } catch(_) {}
        _setHolder(null);
        try { _treg.killByName("su-shell"); } catch(_) {}
        return "ERR: su shell 종료(명령 미완료)";
      }
