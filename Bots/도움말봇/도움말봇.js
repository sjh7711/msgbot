...
var REGISTRY = [
  {
    ...
    commands: [
      ...
      { display: "!출제실패",              triggers: ["!출제실패"],     desc: "반려된 퀴즈 후보 집계 (최근 7일, 사유·토픽별)", admin: true },
      { display: "!출제실패 상세/[토픽]",    triggers: ["!출제실패"],     desc: "반려된 후보의 문제 원문 확인 (최근 5건)", admin: true },
      ...
      { display: "!api보조 [번호]",      triggers: ["!api보조"],      desc: "그 키를 secondary 로 (모든 방, primary 가 한도 초과일 때 사용). 여러 개 지정 가능", admin: true },
      { display: "!api방전용 [번호]",     triggers: ["!api방전용"],     desc: "primary/secondary 를 강등해 등록한 방에서만 쓰이게 함", admin: true }
    ]
  },
  {
    topic: "단어퀴즈봇",
  ...
  },
  {
    ...
    commands: [
      ...
      { display: "!유저등록 [약칭] [알림이름]",      triggers: ["!유저등록"],   desc: "한글 1글자 약칭 등록. 알림이름(카톡에 뜨는 실명)까지 넣어야 !자동기록 대상이 됨", admin: true },
      { display: "!게임등록 [게임키] [알림이름]",     triggers: ["!게임등록"],   desc: "영문 1글자 게임키 등록. 알림이름(예: Patches)까지 넣어야 !자동기록 대상이 됨", admin: true },
      { display: "!등록목록",                      triggers: ["!등록목록"],   desc: "등록된 유저·게임과 각자의 알림이름 확인 (자동기록 대상 여부)", admin: true },
  ...
  {
    topic: "제미니봇",
    aliases: ["제미니", "ㅈㅁㄴ", "gemini", "질문", "ai", "요약", "llm", "내부망", "검색", "링크"],
    commands: [
      { display: "!제미니 [질문] / !ㅈㅁㄴ [질문]", triggers: ["!제미니", "!ㅈㅁㄴ"], desc: "내부망 LLM에 질문 (키 제공자 무제한, 그 외 1일 10회). 최신 정보가 필요하면 웹을 검색해 출처와 함께 답변", admin: false },
      { display: "!제미니 [URL]",          triggers: ["!제미니", "!ㅈㅁㄴ"], desc: "링크를 읽고 한국어로 요약. 예) !제미니 https://... 요약해줘", admin: false }
    ]
  },
  {
    topic: "로그봇",
